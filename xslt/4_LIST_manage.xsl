...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="li[not(ancestor::ul)]">
        <xsl:if test="not(preceding-sibling::*[1][self::li])">
            <xsl:text disable-output-escaping="yes"><![CDATA[<ullist>]]></xsl:text>
        </xsl:if>
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:if test="@type='bull2' and not(preceding-sibling::li [@type='bull1'])">
                <xsl:attribute name="type" select="'bull1'"/>
            </xsl:if>
            <xsl:apply-templates/>
        </xsl:copy>
    ...
    <xsl:template match="list-item [@type='num1'] | list-item [@type='num2'] | list-item [@type='num3'] | list-item [@type='num4']">
    ...
    <xsl:template match="topic">
        <topic>
            ...
            <xsl:attribute name="id" select="concat('d', uuid:get-uuid(title))"/>
            <xsl:apply-templates/>
        </topic>
    </xsl:template>
    ...
    <xsl:function name="uuid:get-uuid" as="xs:string*">
        ...
        <xsl:value-of separator="-" select="substring($ts, 7, 7), substring($ts, 4, 4), string-join((uuid:get-uuid-version(), substring($ts, 1, 3)), ''),             uuid:generate-clock-id(),             uuid:get-network-node()"/>
    </xsl:function>
        ...
        <xsl:choose>
            <xsl:when test="$power &lt; 0 or contains(string($power), '.')">
                <xsl:message terminate="yes">
                    The XSLT template math:power doesn't support negative or fractional arguments.
                </xsl:message>
